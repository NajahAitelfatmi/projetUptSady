...
const Single = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`https://projetuptsady-quxd.onrender.com/api/posts/${postId}`);
        setPost(res.data);
      } catch (err) {
        console.log(err);
      }
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`https://projetuptsady-quxd.onrender.com/api/posts/${postId}`);
      navigate("/h");
    } catch (err) {
      console.log(err);
    }
